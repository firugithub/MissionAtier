...
public class findSubsetsThatSumToATarget {
	 ...
	 *
	 * @param input
	 *            The input array to be processed for subset with particular sum
	 * @param target
	 *            The target sum we are looking for
	 * @param ramp
	 *            The Temporary String to be beefed up during recursive
	 *            iterations(By default value an empty String)
	 * @param index
	 *            The index used to traverse the array during recursive calls
	 */
	public static void findTargetSumSubsets(int[] input, int target, String ramp, int index) {
		...
		// First recursive call going ahead selecting the int at the currenct
		// index value
		findTargetSumSubsets(input, target, ramp + input[index] + token, index + 1);
		// Second recursive call going ahead WITHOUT selecting the int at the
		// currenct index value
